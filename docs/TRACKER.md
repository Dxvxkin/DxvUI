...
| 3 | 🟡 Завершить disabled/фокус/скролл: (а) **сделано** — `EventManager` блокирует клик/фокус/ховер disabled-узлов; (б) **сделано** — наследование «родитель off → дети off»: `isEnabled()` = свой флаг + все предки, `getCurrentState()` под off-предком → `Disabled` (наследуется и визуально — Disabled-стиль применяется ко всему поддереву), `setEnabled` консервативно инвалидирует поддерево (`markStyleDirtyRecursive` + `markLayoutDirtyRecursive`); тесты `EventManagerTest.DisabledAncestor*`, `ChildInsideDisabledParent*`, `DisablingParentReleasesFocusOfFocusedChild`, `ReenablingParentRestoresChildInteraction`; (в) Tab/стрелки-навигация; (г) скроллбары | завершённость по умолчанию | `EventManager`, `SceneNode::isEnabled`, `ScrollContainer` |
