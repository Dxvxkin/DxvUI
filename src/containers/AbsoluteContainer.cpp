...
namespace DxvUI {
    ...
    void AbsoluteContainer::arrange(const Rect& finalRect) {
        ...
        for (const auto& child : children) {
            ...
            int childW = static_cast<int>(childLayout.width > 0 ? childLayout.width : childDesiredSize.width);
            int childH = static_cast<int>(childLayout.height > 0 ? childLayout.height : childDesiredSize.height);
