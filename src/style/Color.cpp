...
Color Color::fromUint32(uint32_t val) {
    return Color(static_cast<uint8_t>((val >> 24) & 0xFF), static_cast<uint8_t>((val >> 16) & 0xFF),
...
Color Color::fromHsv(const Hsv& hsv) {
    ...
    return Color(static_cast<uint8_t>(r_ * 255), static_cast<uint8_t>(g_ * 255),
...
Color Color::lerp(const Color& start, const Color& end, float t) {
    ...
    return Color(static_cast<uint8_t>(start.r + (end.r - start.r) * t),
