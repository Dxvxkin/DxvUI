...
# Единственный источник правды о фетчнутых shared-таргетах: на Windows они дают
# DLL, которые надо разложить рядом с exe (dxvui_deploy_fetched_runtime_dlls), и
# блокируют экспорт пакета (DXVUI_DEPS_EXPORTABLE). Новую shared-зависимость с
# DLL добавлять СЮДА (в точке её фетча), а не в деплой-функцию.
set(DXVUI_FETCHED_SHARED_TARGETS)
...
if(NOT SDL2_FOUND)
    ...
    list(APPEND DXVUI_FETCHED_SHARED_TARGETS SDL2)
else()
...
if(NOT SDL2_ttf_FOUND)
    ...
    list(APPEND DXVUI_FETCHED_SHARED_TARGETS SDL2_ttf)
    if(TARGET freetype)
        list(APPEND DXVUI_FETCHED_SHARED_TARGETS freetype)
    endif()
else()
...
if(NOT spdlog_FOUND)
    ...
    list(APPEND DXVUI_FETCHED_SHARED_TARGETS spdlog)
endif()
...
function(dxvui_deploy_fetched_runtime_dlls target)
    ...
    foreach(name ${DXVUI_FETCHED_SHARED_TARGETS})
        if(TARGET ${name})
            list(PREPEND _dll_paths "$<TARGET_FILE:${name}>")
        endif()
    endforeach()
    list(LENGTH _dll_paths _ndll)
    if(_ndll GREATER 1)
...
if(DXVUI_FETCHED_SHARED_TARGETS)
