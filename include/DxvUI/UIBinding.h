...
namespace DxvUI
{
    ...
    public:
        ...
        [[nodiscard]] std::optional<int> getInt() const noexcept {
            ...
            return std::visit(overloaded{
                [](int arg) -> std::optional<int> { return arg; },
        ...
        [[nodiscard]] std::optional<float> getFloat() const noexcept {
            std::shared_lock<std::shared_mutex> lock(mutex_);
            return std::visit(overloaded{
                [](int arg) -> std::optional<float> { return static_cast<float>(arg); },
        ...
        [[nodiscard]] std::optional<bool> getBool() const noexcept {
            std::shared_lock<std::shared_mutex> lock(mutex_);
            return std::visit(overloaded{
                [](int arg) -> std::optional<bool> { return arg != 0; },
                ...
                [](const std::string& arg) -> std::optional<bool> {
                    ...
                    std::ranges::transform(arg, lower_arg.begin(), [](unsigned char c){ return std::tolower(c); });
    ...
    private:
        ...
        // This helper struct allows std::make_shared to access the private constructor.
        struct MakeSharedEnabler;

        explicit UIBinding(value_t initialValue);
        void unsubscribe(callbackID id);

    public: // Connection must be public to be used in unique_ptr, but its constructor is private to UIBinding
        class Connection {
        public:
            ~Connection();
